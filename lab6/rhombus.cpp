...
void Rhombus::setOriginAtCenter() {
...
void Rhombus::move(sf::Vector2i win, sf::Vector2f offset) {
    ...
    if (boundingBox.left + boundingBox.width > win.x || boundingBox.left < 0) {
        ...
        std::cout << "Rhombus is bounced off horizontal border at pos(" << this->getPosition().x << ", " << this->getPosition().y << ")" << std::endl;
    }
    if (boundingBox.top + boundingBox.height > win.y || boundingBox.top < 0) {
        ...
        offset.x = 0;
        setPosition(getPosition() + offset);
        std::cout << "Rhombus is bounced off vertical border at pos(" << this->getPosition().x << ", " << this->getPosition().y << ")" << std::endl;
    }
}
