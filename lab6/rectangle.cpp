...
void Rectangle::setOriginAtCenter() {
...
void Rectangle::move(sf::Vector2i win, sf::Vector2f offset) {
    ...
    if (boundingBox.left + boundingBox.width > win.x || boundingBox.left < 0) {
        ...
        std::cout << "Rectangle is bounced off horizontal border at pos(" << this->getPosition().x << ", " << this->getPosition().y << ")" << std::endl;
    }
    if (boundingBox.top + boundingBox.height > win.y || boundingBox.top < 0) {
        ...
        offset.x = 0;
        setPosition(getPosition() + offset);
        std::cout << "Rectangle is bounced off vertical border at pos(" << this->getPosition().x << ", " << this->getPosition().y << ")" << std::endl;
    }
}

void Rectangle::move(int winX, int winY, float offsetX, float offsetY) {
    move(sf::Vector2i(winX, winY), sf::Vector2f(offsetX, offsetY));
}

void Rectangle::move(sf::Vector2i win, float offsetX, float offsetY) {
    move(win, sf::Vector2f(offsetX, offsetY));
}

void Rectangle::move(int winX, int winY, sf::Vector2f offset) {
    move(sf::Vector2i(winX, winY), offset);
}

void Rectangle::rotate(float _ang) {
